...
A resolver returns a miss when the target file does not exist; it never fabricates a path. A relative miss therefore warns, and a non-relative miss becomes an external node named by the bare specifier. Concretely: JavaScript and TypeScript probe existing files only, extension probing appends so dotted basenames resolve, a Go package path needs its `.go` file, and a dots-only Python import needs its `__init__.py`.
